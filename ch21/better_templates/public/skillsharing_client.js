...
function displayTalks(talks) {
  talks.forEach(function(talk) {
    ...
    if (talk.deleted) {
      ...
      if (shown) {
        var formBefore = shown.querySelector("input");
        var hasFocus = document.activeElement == formBefore;
        var formBeforeValue = formBefore.value;
        talkDiv.replaceChild(node, shown);
        var formLater = node.querySelector("input");
        formLater.value = formBeforeValue;
        if(hasFocus) formLater.focus();
    }
      else
...
function instantiateTemplate(name, values) {
  ...
  function instantiate(node) {
    ...
    else copy.appendChild(instantiate(child, values));
}
      return copy;
